...
export default function EditPropertyPage() {
  ...
  const [formData, setFormData] = useState<FormData>({
    type: '', purpose: '', zipCode: '', street: '', number: '', complement: '',
    neighborhood: '', city: '', state: '', totalArea: '', privateArea: '',
    bedrooms: '', suites: '', bathrooms: '', parkingSpaces: '', floor: '',
    acceptsPets: false, furnished: false, salePrice: '', rentalPrice: '',
    iptuMonthly: '', condoMonthly: '', description: '',
  });
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      privateArea: property.privateArea != null ? String(property.privateArea) : '',
      ...
      acceptsPets: (property as any).acceptsPets ?? false,
      furnished: (property as any).furnished ?? false,
      salePrice: property.salePrice != null ? String(property.salePrice) : '',
      rentalPrice: property.rentalPrice != null ? String(property.rentalPrice) : '',
      iptuMonthly: property.iptuMonthly != null ? String(property.iptuMonthly) : '',
      condoMonthly: property.condoMonthly != null ? String(property.condoMonthly) : '',
  ...
  async function handleSubmit() {
    try {
      ...
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Erro ao atualizar imóvel.';
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        {property?.code && <span className="text-muted-foreground text-sm">— {property.code}</span>}
      </div>
          ...
          {step === 0 && (
            <>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1.5">
                  <Label>CEP</Label>
                  <Input placeholder="00000-000" value={formData.zipCode} onChange={(e) => setField('zipCode', e.target.value)} />
                </div>
                <div className="col-span-2 space-y-1.5">
                  <Label>Rua</Label>
                  <Input value={formData.street} onChange={(e) => setField('street', e.target.value)} />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1.5">
          ...
          {step === 1 && (
            <>
              ...
              <div className="flex gap-6">
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="acceptsPets" checked={formData.acceptsPets} onChange={(e) => setField('acceptsPets', e.target.checked)} className="w-4 h-4" />
                  <Label htmlFor="acceptsPets">Aceita pets</Label>
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" id="furnished" checked={formData.furnished} onChange={(e) => setField('furnished', e.target.checked)} className="w-4 h-4" />
          ...
          {step === 2 && (
            <>
              ...
              <div className="space-y-1.5">
                ...
                <Textarea rows={4} placeholder="Descreva o imóvel..." value={formData.description} onChange={(e) => setField('description', e.target.value)} />
              </div>
            </>
          )}
          ...
          {step === 3 && (
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                ...
                <div><span className="font-medium">Área Total:</span> {formData.totalArea ? `${formData.totalArea} m²` : '-'}</div>
                ...
                <div><span className="font-medium">Descrição:</span> {formData.description || '-'}</div>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <div className="flex justify-between">
        ...
        {step < STEPS.length - 1 ? (
          <Button onClick={() => setStep((s) => s + 1)} disabled={step === 0 && (!formData.type || !formData.purpose)}>
            Próximo
            <ArrowRight className="w-4 h-4 ml-1" />
          </Button>
        ) : (
          <Button onClick={handleSubmit} disabled={updateProperty.isPending}>
